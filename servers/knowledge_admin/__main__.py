...
The conversational Knowledge tools live in `servers.knowledge` on port 9017.
...
async def knowledge_source_extract(
    ...
    if not manifest.files:
        return {
            "success": False,
            "error": "No source files scanned. Run knowledge_source_scan first.",
        }
...
async def knowledge_source_read(
    ...
    if path not in manifest.files:
        return {
            "success": False,
            "error": f"File '{path}' not in manifest. Run knowledge_source_scan first.",
        }
    ...
    if source.status.value == "needs_vision":
        return {
            ...
            "content": None,
            "hint": (
                "This file needs vision model processing. "
                "Use knowledge_source_convert_pdf to get images."
            ),
        }
