...
run_maintenance() {
    [[ "$RUN_MAINTAIN" -eq 1 ]] || { log "Skipping Knowledge maintenance"; return 0; }
    local py="${PYTHON:-${REPO_DIR}/.venv/bin/python}"
    [[ -x "$py" ]] || py="$(command -v python3)"
    log "Running Knowledge maintenance"
    (cd "$REPO_DIR" && "$py" -m servers.knowledge.maintenance \
        --backup-root "$BACKUP_ROOT" \
        --log-dir "$LOG_DIR" \
        --pending-file "${REPO_DIR}/data/pending_maintenance.json")
}
